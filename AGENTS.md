...
A compiled static website documenting the chronology of **Mário Ferreira dos
Santos (1907–1968)** — Brazilian philosopher, translator, journalist and
publisher of his own books; author of the unfinished *Enciclopédia das Ciências
Filosóficas e Sociais*, and a documented presence in the anarchist Centro de
Cultura Social in São Paulo. It is an intellectual biography: life and works
documented, characterizations attributed, nothing adjudicated.
...
1. **The catalogue is a family and admirer source, and must be labeled every
   time.** The fullest work list reachable is published by the present-day
   admirer-run Instituto Cultural Logos and derives from the philosopher's
   daughters' *Monografia* (2001). Nearly the whole biography passes through the
   same channel, via Pachêco's 2019 UFSC dissertation. That is not a reason to
   refuse it — it is the record that exists — but the site's voice never
   launders it into a neutral bibliography, and `publisherNote` carries the
   label wherever it is cited.
2. **Never assert a volume count for the *Enciclopédia*.** Four figures
   circulate and each has an author: 52 (Olavo de Carvalho), 45 (English
   Wikipedia), "cerca de 50" (Moura & Azevêdo 2015), and the catalogue's
   I–XLVIII including unpublished manuscripts. They are not counting the same
   thing. **No source reached by this project supports a "100+ volume plan"**;
   if you find that claim in a draft, in a source, or in your own memory, it is
   unsupported — say so rather than repeating it.
3. **Per-work years are genuinely slippery.** He self-published and re-edited
   constantly, so an edition year is a commercial fact as much as a
   bibliographic one. Record the first edition you can support, name the known
   re-editions, and flag *every* catalogue-versus-Wikipedia discordance in the
   event that carries the title. The admirer site contradicts *itself* on
   *Filosofia Concreta* — that is data, not an error to tidy away.
4. **Known corrections, already applied — do not regress them.**
   *Invasão Vertical dos Bárbaros* is **1967**, not 1966. "O homem que foi um
   campo de batalha" is an **essay** (written 1943, published 1945 as a preface
   to his Nietzsche translation), **not** a book, and is **not** *O Homem que
   Nasceu Póstumo* (1954). Schooling was the Jesuit **Gymnasio Gonzaga, Pelotas,
   1916–1924**, not the Ginásio Anchieta. No **Kierkegaard** translation appears
   in any catalogue consulted.
5. **Three negative findings are load-bearing.** There is **no DHBB entry** for
   the philosopher (checked with a positive control: the only match was an
   unrelated "Ferreira dos Santos" born 1894). **No estate site is confirmed to
   exist** — `marioferreiradossantos.com`/`.com.br` reset and are absent from
   search indexes; `institutoculturallogos.com` is an admirer site and is not
   one. **No 2007 centenary event was found.** Each is recorded as a checked
   absence with its method; do not silently upgrade any of them to a claim in
   either direction.
6. **Attribute the framings; adjudicate none of them.** "Silenced", "condemned
   to silence", "absolute isolation", "solitary philosopher" are Jorge Jaime's,
   Olavo de Carvalho's and Ladusãns's words; "obstinate independence" is
   Pachêco's reading; a man selling books by the hundred thousand door to door
   complicates both. Write who said it and when. The same applies to the revival
   dispute: Gazeta do Povo's 2018 column is *the column's account*, and É
   Realizações' own "Nota" is **unread** (bot-walled), so its position is not
   characterized here in any direction — not even as disagreement.
7. **Vita's two texts are known only through quotation.** The dismissive 1953
   review and the tepid 1968 obituary reach this project through the secondary
   literature; the *Revista Brasileira de Filosofia* originals were not read.
   Anything said about them beyond the quoted lines is unverified, and must be
   marked as such until someone reads the originals.
8. **The COF corpus is cited, never republished, and never quoted verbatim.**
   He saturates it (queries hit the result limit), so this dataset carries
   **exactly one COF event and one COF fact**, both paraphrase, both naming the
   file ids, dates and review status they rest on (COF014, 2009-07-11,
   *revisada*; COF274, 2014-12-13, *revisada*). A full census is deferred
   follow-up work. REQUIRES-AUDIO discipline applies to every verbatim
   quotation, without exception. Read `KEYWORDS.md` before searching, and
   remember that 32 indexed files are incomplete (archive#37): any zero measured
   here is a lead, not an absence, and every zero reported must name the
   collection, the method and its positive control.
9. **Sources that resisted stay recorded as such.** The access register is in
   `context.md` and in the `publisherNote` of the affected references. A source
   this project could not read is never characterized from its title, and never
   quietly dropped as though it had been checked.
...
The five core rules of the `sourcing-rules` skill govern everything here; the
subject-specific rules that sit on top of them are at the head of this file, and
the contested terrain, the source landscape and the access register are in
`context.md`. Two habits are worth restating because this subject punishes their
absence: **every number about this man is somebody's number** (volumes, copies
sold, years of study), and **every framing of his obscurity is somebody's
framing**. Write the author and the date, or do not write it.
