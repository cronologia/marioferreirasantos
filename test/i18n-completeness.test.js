...
// This repo's dataset is data/chronology.json. The template's
// chronology.example.json stays on disk because two other suites
// (threads.test.js, tier-map.test.js) mutate it as a validator fixture — but it
// is NOT what this site publishes, and pointing the completeness walk at it
// would report a fully translated site while every real string rendered English.
...
//
// Only naming brackets belong here. Everything this dataset has to SAY about a
// source — that a catalogue is family-derived, that a column is one side of a
// dispute, that a page was unreachable — lives in `publisherNote`, which is
// translated. Each id below is listed with the reason its bracket is a name.
const PUBLISHER_BRACKET_OK = new Set([
  // "Revista Filosófica de Coimbra (Universidade de Coimbra)" — the bracket is
  // the journal's parent university, i.e. part of the citation.
  'braga-coimbra-2024',
]);
// <<< ADOPT
